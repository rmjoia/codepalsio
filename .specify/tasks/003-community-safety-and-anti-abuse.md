...
- [ ] **T-311** [US2] Apply Bicep via the CI infra-apply job (PR #46) — verifies the container provisioning works end-to-end.

### Tests (REQUIRED — before implementation)

- [ ] **T-312** [P] [US2] `api/src/lib/reports.test.ts` — repo: create, find-by-id, find-by-reporter+reported (de-dupe key), idempotent insert.
...
- [ ] **T-328** [US3] `api/src/get-roles.ts` — extend to return `[]` for users with `suspended=true` (FR-124).
...
- [ ] **T-348** [US4] `api/src/profiles-list.ts` — extend `PROFILES_QUERY` (or post-process) to filter by mutual block. Performance tradeoff: small in-memory filter on the 200-row page is acceptable; revisit at scale.
