...
4. **Given** I attempt to POST 25 languages, **When** the request is processed, **Then** the server rejects with HTTP 400 (error references the documented max of 10) and the existing stored value is preserved unchanged.
...
- **FR-003**: System MUST cap `spokenLanguages` at a documented maximum (10) per profile. When a submission contains more than 10 entries, the request MUST be rejected with HTTP 400 and the existing stored value MUST be preserved unchanged. The system MUST NOT silently truncate — silent data loss is a worse outcome than an error message. Client UIs MUST prevent over-cap submissions client-side as a usability affordance, but the server contract is reject-and-preserve.
...
| **R3** — Tampered language values: client sends garbage / malicious / oversized strings to inflate filter buckets or cause UI rendering bugs | Medium | Data quality degradation, possible XSS if rendered without escaping | FR-002 (allow-list validation), FR-003 (reject over-cap), FR-010 (server-side only writes), existing CSP `script-src 'self'` (no inline-script attack surface), existing convention that user-controlled strings are inserted into the DOM via `textContent` / element-attribute APIs — `innerHTML` is used only for trusted/static template fragments, never for interpolating user input. |
...
- **SC-002**: /find filter returns correct results within **p95 < 500ms** at the API layer for queries returning ≤ 100 profiles (consistent with `DIRECTORY_PAGE_SIZE = 100` from `api/src/profiles-list.ts:19`). Raising the cap is out of scope for this spec; if profile counts grow such that 100 is too small, that's a separate enhancement spec with its own RU/latency re-evaluation.
